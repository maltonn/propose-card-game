...
const selected_random_card_lst = random_card_lst.sort(() => Math.random() - 0.5).slice(0, 8).concat(["？","！","<空白>","<空白>"])


for (i = 0; i < permanent_card_lst.length+selected_random_card_lst.length; i++) {
    div = document.createElement("div")
    div.className = "card"
    if (i < permanent_card_lst.length) {
        div.innerHTML = permanent_card_lst[i]
        document.getElementById("permanent_card_container").appendChild(div)
    }else{
        div.innerHTML = selected_random_card_lst[i-permanent_card_lst.length]
        document.getElementById("random_card_container").appendChild(div)
    }
